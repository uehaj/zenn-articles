...
- そのために、Claude Code の **PostToolUse hook** で、`.md`を書き出した瞬間に `claude -p` をサブプロセス起動してHTMLを生成させる
- 生成されたHTMLをデフォルトブラウザで開く(確認ダイアログつきで)
