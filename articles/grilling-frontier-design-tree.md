...
つまり将来的には、ハーネスエンジニアリングの少なくない部分が「モデルの頭の中で確実に遵守されるルール」へと置き換わっていくことが予想されます。現在「AI 活用のための高度なフレームワークやテクニック」と見なされている仕組みの多くが、ここ数年のスパンで不要になっていくのではないか——その先進的な予兆を感じさせる鮮やかな例こそが、わずか 20 行のプロンプトでツリーの巡回を行ってみせる mattpocock の grilling なのだと言えます。
