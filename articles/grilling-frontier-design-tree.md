...
本記事で取り上げる [mattpocock/skills](https://github.com/mattpocock/skills) はサードパーティが提供するスキル集であり、頻繁に更新されています。本文の記述は執筆時点(2026年9月、リポジトリの main ブランチおよび手元導入版で確認)の情報に基づいています。最新の内容はリポジトリをご確認ください。
:::
...
grilling のフロンティア方式は、「相互に依存しない質問だけを、ラウンド単位でまとめて提示する」というヒアリングの規律を、20 行程度のプロンプトで実現しています。設計木・フロンティアという語彙から精巧な内部データ構造を想像しがちですが、実体は会話履歴に対する毎ラウンドの再解釈であり、木はデータとして一度も実体化されていません。

このことから、実用上の指針が出ます。長いセッションではコンテキストの要約・圧縮により暗黙の木がズレうるので、確定事項を外部ファイル(ADR・CONTEXT.md)へ明示的に書き出す grill-with-docs 側の運用が安全だ、ということです。
