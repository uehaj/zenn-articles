...
`prefix+u` を押すと、今いるプロジェクトの台帳がポップアップで出ます。

![herdr 0.7.5 のポップアップに開発URL台帳を表示したところ (筆者の実行環境のスクリーンショット)](/images/_urls.png)
...
[herdr](https://herdr.dev) は AI エージェント向けのターミナルマルチプレクサで、任意のコマンドをキーに割り当てる機能をもっています。設定ファイルは `~/.config/herdr/config.toml` です。以下の設定をおこないます。
...
Claude Desktopもいいのですが、こういう小技で使いやすく拡張していけるのはCLIの魅力です。herdrはAPIをもっているところが特徴で、fzfなどをつかうともっとおもしろい小技がつかえます。機会があればご紹介したいところです。
