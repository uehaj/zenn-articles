...
この違いは、`/rewind` と `/branch` の進み方を並べると見えてきます。`/rewind` はセッションを1本に保ち、履歴を巻き戻して別方向へ進みます（B に戻ると、その先にあった C 以降の世界線は消えて戻せません）。`/branch` はセッションそのものを枝分かれさせ、残したどの枝へも `/resume` で行き来できます。
