...
:::message
**2026-08-02 追記**: v2.1.212 で `/fork` の役割が変更されました。本記事で説明している「会話を継承したサブエージェントを裏で起動し、結果がメインに返る」機能は **`/subtask`** コマンドに引き継がれ、現在の `/fork` は「会話をコピーした独立のバックグラウンドセッションを作る」コマンドになっています（結果は自動では返らず、Agent view から回収します。Agent view 無効時のみ従来どおりサブエージェントを起動します）。出典: [changelog v2.1.212](https://github.com/anthropics/claude-code/blob/main/CHANGELOG.md)・[sub-agents ドキュメント](https://code.claude.com/docs/en/sub-agents)。本文中の `/fork` は v2.1.211 以前の挙動としてお読みください。
:::
...
:::message
**2026-08-02 追記**: この節で説明する機能は、v2.1.212 以降 `/subtask` に改名されています。現在の `/fork` は別機能（会話のバックグラウンドセッションへの複製）です。冒頭の追記を参照してください。
:::
