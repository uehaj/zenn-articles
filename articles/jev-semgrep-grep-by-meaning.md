...
また、ここで使う TypeSafe の System One API は公開されて間もないものです。この領域は仕様がかわる可能性もあります。本文の記述は執筆時点(2026年9月, @uehaj/semgrep 0.2.0 / Node v23.10.0 で確認)のものなので、最新の挙動は[公式ドキュメント](https://docs.typesafe.ai/)をご確認ください。
:::
...
:::message
**アイデア: ベクトル検索と組合せる**(まだ実装して確かめたものではありません)

ベクトル検索(およびそれを組み合わせたハイブリッド検索)は、System One モデルによる意味検索と効果的に組み合わせられます。大規模な検索では、まずベクトル検索で候補を粗く絞り、その候補に対して System One モデルで命題の成否を判定する、という二段の検索が、速度と精度の向上において非常に有効になるでしょう。

埋め込みは意味を理解しませんが、大量の候補を事前にインデックス化しておけるので、関連しそうな候補を高速に絞り込めます。問題はその後段です。絞り込んだ結果を LLM で読み直して選別すると、時間もコストもかかります。そこを Jev の意味的な grep に置き換えれば、命題の成否や複合条件(AND / OR / NOT)で機械的に絞り込めます。さらに、確率が閾値付近のグレーゾーンの候補だけを LLM に回して、エージェンティックな RAG 検索で精査する、という三段構成も考えられます。

つまり、RAG の検索パイプラインにおいて、低コスト・速度・精度が高い水準で同時に成立するようになります。
:::
